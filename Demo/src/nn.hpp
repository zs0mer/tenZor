...
class NeuralNet {
  ...
  private:
	Vector<uint64_t> layerSize_;
	std::vector<Matrix<T>> weights_;
	std::vector<Vector<T>> biases_;

	std::vector<Vector<T>> activation_;
	std::vector<Vector<T>> preActivation_;

	// a : activation_
	// z : preActivation_
	std::vector<Vector<T>> dC_da_;
	std::vector<Vector<T>> dC_dz_;

	std::vector<Matrix<T>> weightGrads_;
	std::vector<Vector<T>> biasGrads_;

  public:
	template <class RandomFunc = RrandomUniform<T>>
	NeuralNet(std::initializer_list<uint64_t> layerSize, Device d = CPU,
		...
		for (uint64_t i = 0; i < L; i++) {
			weights_[i] = Matrix<T>(layerSize_[i + 1], layerSize_[i], d);
			biases_[i] = Vector<T>(layerSize_[i + 1], d);

			impl::TensorIMPL<T>::apply(weights_[i].tensor_(), random, impl::AnyDevice{});
			impl::TensorIMPL<T>::apply(biases_[i].tensor_(), random, impl::AnyDevice{});


			weightGrads_[i] = Matrix<T>(layerSize_[i + 1], layerSize_[i], d);
			biasGrads_[i] = Vector<T>(layerSize_[i + 1], d);
	...
	Vector<T> forwardPass(const Vector<T>& input, ActivationFunction activation = {}) {
		...
		for (uint64_t i = 0; i < L; i++) {
			...
			impl::TensorIMPL<T>::apply(preActivation_[i + 1].tensor_(),
			                           activation_[i + 1].tensor_(), activation, impl::AnyDevice{});
		}
	...
	void backwardPass(const Vector<T>& input, const Vector<T>& target,
		...
		for (uint64_t i = L; i-- > 0;) {
			...
			impl::TensorIMPL<T>::apply(preActivation_[i + 1].tensor_(), dC_dz_[i + 1].tensor_(),
			                           activationFunctionDerivative, impl::AnyDevice{});
